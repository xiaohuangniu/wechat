...
/*
 +----------------------------------------------------------------------
 + Title        : PHP微信开发 生成带参数的二维码 - 永久版
 + Author       : 小黄牛
 + Version      : 无
 + Initial-Time : 2016-10-29 09:01:00
 + Last-time    : 2016-10-29 09:01:00 + 小黄牛
...
class WeiXin{
	...
	public function Temporary_QR(){
		...
		# 设置二维码参数
		$data = array(
			'action_name'    => 'QR_LIMIT_SCENE',//二维码类型，QR_LIMIT_SCENE为永久
			# 场景参数
			'action_info'    => array(
				'scene' => array(
					'scene_id'   => 456 // 场景值ID，临时二维码时为32位非0整型，永久二维码时最大值为100000（目前参数只支持1--100000）
				)
			)
		);
		$result = self::https_request($url,json_encode($data));//参数一定要转成JSON
		$this->Error_Log(array('ticket参数'=>$result),true);//记录日志,以防报错
